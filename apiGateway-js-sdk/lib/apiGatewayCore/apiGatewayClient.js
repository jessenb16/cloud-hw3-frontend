...
apiGateway.core.apiGatewayClientFactory.newClient = function (simpleHttpClientConfig, sigV4ClientConfig) {
    ...
    apiGatewayClient.makeRequest = function (request, authType, additionalParams, apiKey) {
        ...
        // Check if body should be undefined, but preserve ArrayBuffer and other binary types
        if (request.body === undefined || request.body === '' || request.body === null) {
            request.body = undefined;
        } else if (request.body instanceof ArrayBuffer || request.body instanceof Blob || request.body instanceof File || request.body instanceof Uint8Array) {
            // Preserve binary data types - don't set to undefined
            // File extends Blob, so it's included here
            // These types are valid and should be sent
        } else if (typeof request.body === 'object' && Object.keys(request.body).length === 0) {
            // Only set to undefined if it's an empty plain object
            request.body = undefined;
        }
